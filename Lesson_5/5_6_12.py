...
1. Открыть ссылку с каждым cookie из списка, достать информацию о возрасте и скиллах.
2. Выбрать самого молодого и знающего больше всего языков программирования.
3. Вывести на экран "value" из его cookie.
"""
...
class Locators:
    skills = By.CSS_SELECTOR, "#skillsList > li"
    age = By.ID, "age"


def cookie_quiz():
    with webdriver.Chrome() as browser:
        max_skillslength, min_age, winner = 0, 100, None
        browser.get(URL)
        for cookie in COOKIES:
            browser.add_cookie(cookie)
            browser.refresh()
            person_skills = len(browser.find_elements(*Locators.skills))
            age_info = browser.find_element(*Locators.age).text
            person_age = int(age_info.split()[1])
            if person_skills > max_skillslength and person_age < min_age:
                max_skillslength, min_age = person_skills, person_age
                winner = cookie['value']
